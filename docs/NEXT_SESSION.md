...
## Step 1 — de-risk FIRST. Do not skip this.

Before building anything, prove the quality clears the owner's bar. Build one
Kaggle notebook that produces **three sample clips** and nothing else:
...
Then stop and show them. Roughly 20 GPU-minutes.
...
Model guidance for a free T4 (16 GB):
...
- The build ran in a cloud container whose egress policy blocked
  `kaggle.com`, `huggingface.co` and `commons.wikimedia.org`. That is why the
  Kaggle jobs and the Wikimedia photo fetch are marked untested in `PLAN.md`.
  **On a normal machine they should work** — verify them early rather than
